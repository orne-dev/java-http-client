...
/**
 * Unit tests for {@code JsonHttpResponseBodyParser}.
 * 
 * @author <a href="mailto:wamphiry@orne.dev">(w) Iker Hernaez</a>
 * @version 1.0, 2023-07
 * @since 0.1
 * @see JsonHttpResponseBodyParser
 */
@Tag("ut")
class JsonHttpResponseBodyParserTest
extends HttpResponseBodyMediaTypeParserTest {
    ...
    protected @NotNull JsonHttpResponseBodyParser<?> createParser() {
        return spy(JsonHttpResponseBodyParser.class);
    }

    /**
     * Test for {@link JsonHttpResponseBodyParser#getDefaultContentType()}.
     * @throws Throwable Should not happen
     */
    @Test
    void testGetDefaultContentType()
    throws Throwable {
        final JsonHttpResponseBodyParser<?> parser = createParser();
        final ContentType result = parser.getDefaultContentType();
        assertNotNull(result);
        assertEquals(MediaTypes.Application.JSON, result.getMediaType());
        assertEquals(StandardCharsets.UTF_8, result.getCharset());
    }

    /**
     * Test for {@link JsonHttpResponseBodyParser#supportsMediaType(String)}.
     * @throws Throwable Should not happen
     */
    @Test
    ...
    throws Throwable {
        final JsonHttpResponseBodyParser<?> parser = createParser();
        assertTrue(parser.supportsMediaType(MediaTypes.Application.JSON));
        assertTrue(parser.supportsMediaType(MediaTypes.Application.CALENDAR_JSON));
        assertTrue(parser.supportsMediaType(MediaTypes.Application.GEO_JSON));
        assertTrue(parser.supportsMediaType(MediaTypes.Application.JSON_PATCH));
        assertTrue(parser.supportsMediaType(MediaTypes.Application.MANIFEST_JSON));
        assertTrue(parser.supportsMediaType(MediaTypes.Application.MERGE_PATCH_JSON));
        assertTrue(parser.supportsMediaType(MediaTypes.Application.VCARD_JSON));
    }
    ...
    throws Throwable {
        final boolean expected = MediaTypes.Application.JSON.equals(mediaType) ||
